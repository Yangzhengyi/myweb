function SpaceCount(UserName)
 {
   var SpaceCookie=GetCookie(UserName);
   if(SpaceCookie=="1")
    {
      return;
    }
  else
   {
     var x=new PAAjax();
     x.setarg("post",true);
     x.send("/e/space/spacecount.aspx","username="+UserName,function(v){Set_SpaceCookie(UserName)});
    }
 }

function Set_SpaceCookie(UserName)
 {
   SetCookie(UserName,"1",24*60);
 }
...
function SpcLogin() //用户留言登录
 ...
 function r_spcfbk(did) //留言回复
  {
    ShowIframe('留言回复','/e/space/fbk_reply.aspx?id='+did,450,200);
  }