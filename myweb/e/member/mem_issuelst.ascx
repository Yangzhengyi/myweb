      ...
      <asp:Repeater id="P1" runat="server" OnItemDataBound="Data_Bound">         
             <ItemTemplate>
                 <tr>
                  ...
                  <td align=center  class="memlist_item_last"><a href=# onclick="return Issue('<%#DataBinder.Eval(Container.DataItem,"thetable")%>','<%#DataBinder.Eval(Container.DataItem,"id")%>','<%#DataBinder.Eval(Container.DataItem,"work_id")%>')">签发</a></td>
                </tr>
             </ItemTemplate>
          </asp:Repeater>
 </table>
 ...
 function Issue(Table,Id,Wid)
  {
   ...
   var Val=window.open("/e/member/issue.aspx?s="+Sid+"&table="+Table+"&id="+Id+"&workid="+Wid,"issue","width="+Width+",height="+Height+",top="+Top+",left="+Left+",toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=yes");
   return false;
 } 
