 ...
 <table border=0 cellpadding=5  width=95% align=center>
    ...
    <tr>
     <tr>
         ...
         <td><select  name="Sort" id="Sort"><option  value="0">-----选择导入类别------</option><%=Sort_List%></select></td>
     </tr>
    <tr>
     <tr>
    ...
    if(obj_urls.value.indexOf("http://")>=0)
     {
       ...
       sendurl="collection_5.aspx?sortid="+objsort.value+"&checked="+document.getElementById("DefaultChecked").value+"&saveimage=<%=C_detail_saveimage%>&table=<%=Table%>&cid=<%=Cid%>&c_list_domain=<%=C_list_domain%>&c_encode=<%=C_encode%>&collection=yes&siteid=<%=SiteId%>";
