   ...
   if(Ids==null || !IsNum(Ids.Replace(",","")) || Table=="pa_sublanmu" || Table=="pa_lanmu")
    {
      ...
      if(IsDate(D1) && IsDate(D2))
       ...
       switch(Table)
         {
           case "pa_zt":
            sort_ids=Request.QueryString["sortid"];
           ...
           sql="select id from "+Table+" where "+SiteTj+TJ+DateTj;
           countsql="select count(id) as co from "+Table+" where "+SiteTj+TJ+DateTj;
           Ids="";
           break;
           ...
           case "pa_lanmu":
             ...
             if(LanmuIds==null || !IsNum(LanmuIds.Replace(",","")))
              {
               sql="select id from pa_lanmu where "+SiteTj+" and thetype<>'external'";
               countsql="select count(id) as co from pa_lanmu where "+SiteTj+" and thetype<>'external'";
              }
             else
              {
           ...
           case "pa_sublanmu":
             ...
             if(SubLanmuIds!=null && IsNum(SubLanmuIds.Replace(",","")) )
              ...
              {
               sql="select id from pa_sublanmu where "+SiteTj;
               countsql="select count(id) as co from pa_sublanmu where "+SiteTj;
              }
             Ids="";
           break;
           ...
           default:
            ...
            IsDetail="1";
            sql="select id from "+Table+" where "+SiteTj+" and checked=1 and html=2 "+TJ+DateTj;
            countsql="select count(id) as co from "+Table+" where "+SiteTj+" and checked=1 and html=2 "+TJ+DateTj;
            Ids="";
           break;
         }
     Tj(countsql);
...
<br>
<div align="center"><input type="button" id="bt_stop" value="暂停" class="button" onclick="stop_build()"> &nbsp; <input type="button" value="关闭" class="button" onclick="parent.ClosePop()"></div>
...
function start_html(Ids)
 {
   ...
   //window.open("build_static.aspx?table="+Table+"&ids="+Ids"));
}
