...
<table  border=0 cellpadding=0 cellspacing=0 width=95% >
 ...
 <tr><td class=table_style1><b>子栏目管理</b></td></tr>
 <tr><td height=10></td></tr>
</table>
...
<li id="tab" name="tab" onclick="location.href='sublanmu_add.aspx?lanmuid=<%=Request.QueryString["lanmuid"]%>&navid=<%=Request.QueryString["navid"]%>&lanmuname=<%=Server.UrlEncode(Request.QueryString["lanmuname"])%>&navname=<%=Server.UrlEncode(Request.QueryString["navname"])%>'">增加子栏目</li>
</ul>
</div>
...
<tr>
  <td  colspan=2 height=25><b>当前位置：</b><a href=lanmu_list.aspx>栏目管理(<%=Request.QueryString["lanmuname"]%>)</a> &gt; 子栏目管理</td>
 </tr>
</table>
    ...
    <table border=0 cellpadding=0 cellspacing=2 width=98% align=center>
      <tr>
        <td  colspan=2 height=25>
            <table border=0 cellpadding=0 cellspacing=0 width=100% class=tablestyle id="s_list">
                <tr>
...
<select name="toid" id="toid"><option value="0">根栏目</option><%=Options_List%></select>
...
function Module_Set(Id)
 {
  ...
  location.href="module_list.aspx?lanmuid=<%=Request.QueryString["lanmuid"]%>&lanmuname=<%=Server.UrlEncode(Request.QueryString["lanmuname"])%>&navid=<%=Request.QueryString["navid"]%>&navname=<%=Server.UrlEncode(Request.QueryString["navname"])%>&sublanmuid="+Id;
 }
...
function Set(Id,thetable)
 {
  ...
  window.open(url,"sublanmuset","width="+Width+",height="+Height+",top="+Top+",left="+Left+",toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=yes");
...
function build_htm()
 {
  ...
  var Width=400;
  var Height=180;
  ShowIframe("子栏目页静态生成","static_panel.aspx?table=pa_sublanmu&lanmuids=<%=Lanmu_Id%>&sublanmuids="+Ids,Width,Height,'no');
 }

function back()
 {
  var curl="nav_list.aspx?lanmuid=<%=Request.QueryString["lanmuid"]%>&lanmuname=<%=Server.UrlEncode(Request.QueryString["lanmuname"])%>";
  location.href=curl;
 }
</script>
</html>  
