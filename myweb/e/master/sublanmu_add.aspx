...
<div id="tabdiv">
<ul>
<li id="tab" name="tab" onclick="location.href='sublanmu_list.aspx?lanmuid=<%=Request.QueryString["lanmuid"]%>&navid=<%=Request.QueryString["navid"]%>&lanmuname=<%=Server.UrlEncode(Request.QueryString["lanmuname"])%>&navname=<%=Server.UrlEncode(Request.QueryString["navname"])%>'">子栏目管理</li>
...
<table border=0 cellpadding=5 cellspacing=2 width=95% align=center>
  <tr>
  <td  height=25>父属栏目</td>
  <td> <select size="10" name="Parent_SubLanmu" id="Parent_SubLanmu" style="width:300px;">
	<option value="0|0|0|"'>根栏目</option>
        <%=Parent_List%>
       </select>
  </td>
 </tr>
  ...
  <tr id="Tr_Dir">
  <td  height=25 width=100px>静态生成路径</td>
  <td title="必填项，只能由字母、数字或下划线组成">/父目录/<input name="SubLanmu_Dir" type="text" maxlength="30" id="SubLanmu_Dir" size="15" ><input name="SubLanmu_File" type="hidden" id="SubLanmu_File" value="index.html"></td>
  </tr>
...
<input type="hidden" name="Lanmu_Dir"  value="<%=Lanmu_Dir%>">
<input type="hidden" name="post"   value="add">
<span id="post_area">
<input type="submit" class=button value="提交" onclick="return C_Form()" id="Bt_Submit">
<input type="button" class=button value="返回"  onclick="location.href='sublanmu_list.aspx?lanmuid=<%=Request.QueryString["lanmuid"]%>&navid=<%=Request.QueryString["navid"]%>&lanmuname=<%=Server.UrlEncode(Request.QueryString["lanmuname"])%>&navname=<%=Server.UrlEncode(Request.QueryString["navname"])%>'">
</span>
