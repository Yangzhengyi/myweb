...
<table  border=0 cellpadding=0 cellspacing=0 width=95% >
 ...
 <tr><td class=table_style1><b>信息列表</b></td></tr>
 <tr><td height=10 ></td></tr>
</table>
...
</select><input text="text" id="s_keyword" size="15">
          ...
          <asp:Repeater id="P1" runat="server" OnItemDataBound="Data_Bound">         
             <ItemTemplate>
                 <tr title="<%#Server.HtmlEncode(DataBinder.Eval(Container.DataItem,"title").ToString())%>">
                  <td align=left class="tdstyle"><input type="checkbox" id="CK" Name="CK" Value="<%#DataBinder.Eval(Container.DataItem,"id")%>"><%#GetSortName(DataBinder.Eval(Container.DataItem,"sort_id").ToString())%><a href='<%#DetailUrl(DataBinder.Eval(Container.DataItem,"static_dir").ToString(),DataBinder.Eval(Container.DataItem,"static_file").ToString(),DataBinder.Eval(Container.DataItem,"lanmu_id").ToString(),DataBinder.Eval(Container.DataItem,"subLanmu_id").ToString(),DataBinder.Eval(Container.DataItem,"id").ToString(),DataBinder.Eval(Container.DataItem,"zdy_url").ToString(),DataBinder.Eval(Container.DataItem,"permissions").ToString(),DataBinder.Eval(Container.DataItem,"checked").ToString(),DataBinder.Eval(Container.DataItem,"html").ToString())%>' target="dataview"><%#SubStr(DataBinder.Eval(Container.DataItem,"title").ToString(),100,true)%></a><img src=images/image.gif height=20px align=absmiddle title="带标题图片" style="display:<%#(DataBinder.Eval(Container.DataItem,"titlepic").ToString())==""?"none":""%>"><asp:Label ForeColor="#ff0000" text="[顶]"  visible='<%#Get_Bool(DataBinder.Eval(Container.DataItem,"istop").ToString())%>' Title='<%#"置顶结束日期："+(DataBinder.Eval(Container.DataItem,"actdate")).ToString()%>' runat="server"/><asp:Label ForeColor="#ff0000" text="[荐]"  visible='<%#Get_Bool(DataBinder.Eval(Container.DataItem,"isgood").ToString())%>' runat="server"/><asp:Label ForeColor="#ff0000" text="[新]"  visible='<%#Get_Bool(DataBinder.Eval(Container.DataItem,"isnew").ToString())%>' runat="server"/><asp:Label ForeColor="#ff0000" text="[热]"  visible='<%#Get_Bool(DataBinder.Eval(Container.DataItem,"ishot").ToString())%>' runat="server"/><a href="javascript:Sign_Data('<%=Request.QueryString["table"]%>','<%#DataBinder.Eval(Container.DataItem,"id")%>')" style="display:<%#DataBinder.Eval(Container.DataItem,"sign_open").ToString()=="1"?"":"none"%>"><span style="color:#ff0000">[签收查看]</span><a>
                  </td>
                  ...
                  <td align=center class=tdstyle><a href="javascript:Issue_Data('<%#Request.QueryString["table"]%>','<%#DataBinder.Eval(Container.DataItem,"id")%>','<%#DataBinder.Eval(Container.DataItem,"work_id")%>')"><%#Get_CheckState(DataBinder.Eval(Container.DataItem,"checked").ToString(),DataBinder.Eval(Container.DataItem,"work_id").ToString(),DataBinder.Eval(Container.DataItem,"work_state").ToString())%></a></td>
                  ...
                  <td align=center class="tdstyle">
                   <a href='data_reply.aspx?id=<%#DataBinder.Eval(Container.DataItem,"id")%>&table=<%#Request.QueryString["table"]%>&name=<%#Server.UrlEncode(Request.QueryString["name"])%>'>回复</a>
                   <a href='data_add.aspx?id=<%#DataBinder.Eval(Container.DataItem,"id")%>&table=<%#Request.QueryString["table"]%>&name=<%#Server.UrlEncode(Request.QueryString["name"])%>'>编辑</a>
...
<script type="text/javascript">
 ...
 function Go()
  { 
   ...
   location.href="?table="+Table+"&name="+escape(name)+"&sortid="+obj_sort.value+"&from="+obj_from.value+"&type="+obj_type.value+"&field="+obj_field.value+"&order="+obj_order.value+"&keyword="+escape(obj_keyword.value)+"&pagesize="+obj_pagesize.value;
  }
  ...
  if(act=="pset")
    {
      ...
      ShowIframe("批量参数设置","data_set.aspx?table="+Table+"&name="+encodeURI("<%=Request.QueryString["name"]%>"),650,450);
      return;
    }
...
function htm_panel(Table,Name,Ids)
 {
  var Width=400;
  var Height=180;
  ShowIframe("内容页静态生成","static_panel.aspx?table="+Table+"&ids="+Ids+"&sortid=<%=Request.QueryString["sortid"]%>",Width,Height,'no');
 }
SetCookie("tab","0");
