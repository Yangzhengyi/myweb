...
<div class="spc_location">当前位置：<a href="/e/space/?s=<%=Request.QueryString["s"]%>&username=<%=Request.QueryString["username"]%>"><%=Request.QueryString["username"]%>的空间</a> &gt; 我的留言</div>
...
<table border=0 cellpadding="5px" cellspacing=0  align=center width="99%">
  <form name="f" method="post" action="index.aspx?s=<%=Request.QueryString["s"]%>&username=<%=Request.QueryString["username"]%>&type=fbk">
   <tr>
  ...
  <table border=0 cellpadding="5px" cellspacing=0  align=center width="98%">
    <form name="l" method="post">
     <tr>
      ...
      <input type="submit" value="登录"  onclick="return SpcLogin()" class="bt">&nbsp;&nbsp;[<a href="/e/member/index.aspx?s=<%=Request.QueryString["s"]%>&type=reg">没有账号点此注册</a>]
     </td>
    </tr>
   </form>
 </table>
...
<script type="text/javascript" src="/e/js/popup.js"></script>
...
<div class="module_content">
 <ul class="fbklist">
  <asp:Repeater Id="Plist" Runat="server" OnItemDataBound="Data_Bound">
    <ItemTemplate>
    <li class="left finfo"><a href="/e/space/?s=1&username=<%#DataBinder.Eval(Container.DataItem,"fbk_username")%>" target="space"><%#Get_TrueName(DataBinder.Eval(Container.DataItem,"fbk_username").ToString())%></a> <span class="finfo">发布于：<%#DataBinder.Eval(Container.DataItem,"fbk_date","{0:yyyy-MM-dd HH:mm:ss}")%></span></li>
