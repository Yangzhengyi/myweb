...
private void Get_Data() //获取资料
 {
   sql="select site_id,sort_id from "+Thetable+" where id="+DetailId;
   OleDbCommand comm=new OleDbCommand(sql,conn);
   OleDbDataReader dr=comm.ExecuteReader();
   if(dr.Read())
    {
      SiteId=dr["site_id"].ToString();
      Get_ActPoint(dr["sort_id"].ToString());
    }
   else
    {
      SetTop();
...
protected void Get_ActPoint(string sortid) //获取单位扣除积分
 {
   ActPoint=0;
   sql="select act_point from pa_sort where id="+int.Parse(sortid);
   OleDbCommand myComm=new OleDbCommand(sql,conn);
   OleDbDataReader dr=myComm.ExecuteReader();
   if(dr.Read())
    {
      ActPoint=int.Parse(dr["act_point"].ToString());
    }
   else
    {
      Response.End();
...
private void SetTop()
  {
   ...
   conn.Close();
   Response.Write("<s"+"cript type='text/javascript'>alert('置顶成功!');parent.ClosePop('refresh')</"+"script>");
   Response.End();
  }
